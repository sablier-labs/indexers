...
import { loadProxender } from "../../helpers/lockup";
import {
  inferDynamicShape,
  inferLinearShape,
  inferTranchedShape,
} from "../../helpers/lockup/shape-inference";
import { Params, Segment, Tranche } from "../../helpers/lockup/types";
...
export function getStream(tokenId: BigInt): Entity.LockupStream | null {
  const id = Id.stream(dataSource.address(), tokenId);
  return Entity.LockupStream.load(id);
}
...
function createBaseStream(
  ...
  const stream = new Entity.LockupStream(streamId);

  /* --------------------------------- WATCHER -------------------------------- */
  const watcher = getOrCreateWatcher();
  stream.subgraphId = watcher.lockupStreamCounter;
...
function addDynamicShape(stream: Entity.LockupStream, segments: Segment[]): Entity.LockupStream {
  if (stream.shape !== null) {
    stream.shapeSource = "Event";
    return stream;
  }
...
function addSegments(stream: Entity.LockupStream, segments: Segment[]): Entity.LockupStream {
  let streamed = ZERO;
...
function addTranchedShape(stream: Entity.LockupStream, tranches: Tranche[]): Entity.LockupStream {
  if (stream.shape !== null) {
    stream.shapeSource = "Event";
    return stream;
  }
...
function addTranches(stream: Entity.LockupStream, tranches: Tranche[]): void {
