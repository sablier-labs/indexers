 ...
 */
export function createOrCreateBatch(event: ethereum.Event, sender: Address): Entity.LockupBatch {
  const id = Id.batch(event, sender);
  const batcher = getOrCreateBatcher(sender);

  let batch = Entity.LockupBatch.load(id);
  if (batch === null) {
    batch = new Entity.LockupBatch(id);
    batch.size = ONE;
  } else {
