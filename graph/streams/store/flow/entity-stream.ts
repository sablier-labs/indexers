...
export function createStream(
  ...
  const stream = new Entity.FlowStream(streamId);
  const watcher = getOrCreateWatcher();

  /* --------------------------------- WATCHER -------------------------------- */
  stream.subgraphId = watcher.flowStreamCounter;
...
export function getStream(tokenId: BigInt): Entity.FlowStream | null {
  const flowAddress = dataSource.address();
  const id = Id.stream(flowAddress, tokenId);
  return Entity.FlowStream.load(id);
}
