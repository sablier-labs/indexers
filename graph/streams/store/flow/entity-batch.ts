 ...
 */
export function createOrUpdateBatch(event: ethereum.Event, sender: Address): Entity.FlowBatch {
  const id = Id.batch(event, sender);
  const batcher = getOrCreateBatcher(sender);

  let batch = Entity.FlowBatch.load(id);
  if (batch === null) {
    batch = new Entity.FlowBatch(id);
    batch.size = ONE;
  } else {
