...
Contract.Airdrops.Campaign.MerkleInstant_v1_3.Clawback.handler(async ({ context, event }) => {
  await Store.User.createOrUpdate(context, event, [event.params.admin, event.params.to, event.transaction.from]);
});

Contract.Airdrops.Campaign.MerkleInstant_v1_3.TransferAdmin.handler(async ({ context, event }) => {
