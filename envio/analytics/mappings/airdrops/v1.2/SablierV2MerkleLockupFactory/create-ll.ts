...
Contract.Airdrops.Factory.MerkleLockupFactory_v1_2.CreateMerkleLL.contractRegister(({ context, event }) => {
...
Contract.Airdrops.Factory.MerkleLockupFactory_v1_2.CreateMerkleLL.handler(async ({ context, event }) => {
  const lockupAddress = event.params.lockupLinear;
  if (!isOfficialLockup(context.log, event, lockupAddress, { allowAll: true })) {
    return;
  }
