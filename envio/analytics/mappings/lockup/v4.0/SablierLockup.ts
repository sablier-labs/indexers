...
Contract.Lockup.Lockup_v4_0.CancelLockupStream.handler(async ({ context, event }) => {
...
Contract.Lockup.Lockup_v4_0.RenounceLockupStream.handler(async ({ context, event }) => {
  await Store.User.createOrUpdate(context, event, [event.transaction.from]);
});

Contract.Lockup.Lockup_v4_0.WithdrawFromLockupStream.handler(async ({ context, event }) => {
  await Store.User.createOrUpdate(context, event, [event.params.to, event.transaction.from]);
  // Lockup v4.0 maintains fees tracking
...
Contract.Lockup.Lockup_v4_0.Approval.handler(async ({ context, event }) => {
...
Contract.Lockup.Lockup_v4_0.ApprovalForAll.handler(async ({ context, event }) => {
...
Contract.Lockup.Lockup_v4_0.Transfer.handler(async ({ context, event }) => {
