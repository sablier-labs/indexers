...
@genType
and entities = {
    @as("Revenue") revenue: entityStoreOperations<Entities.Revenue.t>,
    @as("RevenueTransaction") revenueTransaction: entityStoreOperations<Entities.RevenueTransaction.t>,
    @as("User") user: entityStoreOperations<Entities.User.t>,
    @as("UserTransaction") userTransaction: entityStoreOperations<Entities.UserTransaction.t>,
  }
  ...
  let entities = {
      revenue: {
        makeStoreOperatorEntity(
          ...
          ~getStore=db => db->InMemoryStore.getInMemTable(
            ~entityConfig=module(Entities.Revenue)->Entities.entityModToInternal,
          )->(
            Utils.magic: InMemoryTable.Entity.t<Internal.entity> => InMemoryTable.Entity.t<
              Entities.Revenue.t,
            >
      ...
      revenueTransaction: {
        makeStoreOperatorEntity(
          ...
          ~makeMockDb=makeWithInMemoryStore,
          ~getStore=db => db->InMemoryStore.getInMemTable(
            ~entityConfig=module(Entities.RevenueTransaction)->Entities.entityModToInternal,
          )->(
            Utils.magic: InMemoryTable.Entity.t<Internal.entity> => InMemoryTable.Entity.t<
              Entities.RevenueTransaction.t,
            >
          ),
          ~getKey=({id}) => id,
        )
      ...
      userTransaction: {
        makeStoreOperatorEntity(
          ...
          ~makeMockDb=makeWithInMemoryStore,
          ~getStore=db => db->InMemoryStore.getInMemTable(
            ~entityConfig=module(Entities.UserTransaction)->Entities.entityModToInternal,
          )->(
            Utils.magic: InMemoryTable.Entity.t<Internal.entity> => InMemoryTable.Entity.t<
              Entities.UserTransaction.t,
            >
          ),
          ~getKey=({id}) => id,
        )
...
and makeProcessEvents = (mockDb: t, ~chainId=?) => async (
  ...
  let persistence = {
    ...
    storageStatus: Ready({cleanRun: false, cache: Js.Dict.empty()}),
  }
  let config = {
    ...config,
    persistence,
  ...
  try {
    await eventItems->EventProcessing.preloadBatchOrThrow(
      ~loadManager,
      ~persistence,
      ~inMemoryStore,
    )
...
and makeMockStorage = (mockDb: t): Persistence.storage => {
  {
    ...
    restoreEffectCache: (~withUpload as _) => Js.Exn.raiseError("Not used yet"),
  }
}
and /**
